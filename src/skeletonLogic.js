        ...
        k.state("patrol-right", ["patrol-right", "patrol-left", "attack", "alert", "retreat"]),
        {
            ...
            setBehaviour() {
                ...
                this.onStateUpdate("attack", () => {//We want to attack state to take place without any entering in the state yet we can write it but we want it to start it immediately
                    ...
                    this.flipX = player.pos.x <= this.pos.x;
                    this.moveTo(k.vec2(player.pos.x + 55, player.pos.y), this.pursuitSpeed)//Function offered by kaboom to body containing elements it takes position and then a speed 
                })
                ...
                this.onUpdate(() => {
                    ...
                    if (this.pos.dist(player.pos) <= 63 && this._attackCooldown <= 0) {
                        ...
                        const skeletonHitBox = this.add([
                            ...
                            k.area({ shape: new k.Rect(k.vec2(offX, -30), 70, 45) }),
                            "creature-hitbox",
                        ]);

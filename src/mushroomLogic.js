export function makeMushroom(k, initialPos) {
    return k.make([
        k.pos(initialPos),
        k.sprite("mushroom-walk", { anim: "walk" }),
        k.area({
            shape: new k.Rect(k.vec2(0, 10), 20, 35),
        ...
        k.state("patrol-right", ["patrol-right", "patrol-left", "attack", "alert", "retreat"]),
        {
            ...
            setBehaviour() {
                ...
                this.onStateEnter("alert", async () => {
                    this.use(k.sprite("mushroom-idle"))
                    this.play("idle");
                    this.flipX = player.pos.x <= this.pos.x;
                    await k.wait(1);
                    this.use(k.sprite("mushroom-walk"))
                    this.play("walk")
                    if (this.pos.dist(player.pos) < this.range) {
                ...
                this.onStateUpdate("attack", () => {//We want to attack state to take place without any entering in the state yet we can write it but we want it to start it immediately
                    ...
                    this.flipX = player.pos.x <= this.pos.x;
                    this.moveTo(k.vec2(player.pos.x + 30, player.pos.y), this.pursuitSpeed)//Function offered by kaboom to body containing elements it takes position and then a speed 
                })
            ...
            setEvents() {
                ...
                this.onUpdate(() => {
                    ...
                    if (this.pos.dist(player.pos) <= 33 && this._attackCooldown <= 0) {
                        ...
                        const mushroomHitBox = this.add([
                            ...
                            k.area({ shape: new k.Rect(k.vec2(offX, -25), 30, 45) }),
                            "creature-hitbox",
                        ]);
                        // destroy hitbox after animation window
                        k.wait(0.7, () => {
                            if (mushroomHitBox && mushroomHitBox.exists()) mushroomHitBox.destroy();
                            this.use(k.sprite("mushroom-walk"));
                            this.play("walk");
                            this._isPerformingAttack = false;
                        });
                ...
                this.on("die", () => {
                    ...
                    this.unuse("body")
                    this.use(k.sprite("mushroom-death"))
                    this.play("death");
                    k.wait(0.5, () => {
                ...
                this.onCollide("attack-hitbox", () => {
                    this.hurt(1);
                    this.use(k.sprite("mushroom-hit"))
                    this.play("hit")
                    k.wait(0.3, () => {
                        if (this.hp() === 0) {
                            this.trigger("die");

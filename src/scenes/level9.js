...
import { makeBoss } from "../boss7Logic.js"
import { setColliders, setCameraZones, setCameraControls, setEntryAndExitPoints } from "./commonScriptForLevels.js"
export function level9(k, level9Data) {
    k.camScale(1);
    k.camPos(400, 290);
    k.setGravity(1000);
    const levellayers = level9Data.layers;

    const map = k.add([k.pos(0, 0), k.sprite("level9")])
    const colliders = []
    const positions = []
    const cameras = []
    const exits = []
    ...
    const player = map.add(makePlayer(k, level9Data));
    //Creating the boss
    const boss = map.add(makeBoss(k, player))

    setCameraControls(k, map, player, level9Data);

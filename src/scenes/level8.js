...
export function level8(k, level8Data) {
    k.camScale(1);
    k.camPos(400, 290);
    k.setGravity(1000);
    const levellayers = level8Data.layers;

    const map = k.add([k.pos(0, 0), k.sprite("level8")])
    const colliders = []
    const positions = []
    const cameras = []
    const exits = []
    ...
    const player = map.add(makePlayer(k, level8Data));
    //Creating the boss
    const boss = map.add(makeBoss(k, player))

    setCameraControls(k, map, player, level8Data);
